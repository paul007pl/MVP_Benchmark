...
class NM(nn.Module):
    def __init__(self, radius=0.1, max_nn=10):
...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.k_ppf = args.size_ppf

        if self.use_ppf:
            self.emb_nm = NM()
            self.hie1 = nn.Sequential(
                Conv1dBNReLU(4*self.k_ppf, 64),
            # 	Conv1dBNReLU(64, 64),
                # Conv1dBNReLU(64, 64),
                Conv1dBNReLU(64, 128)
                )
    ...
    def get_ds_cent_feats(feats, points, num_centers):
        p_idx = furthest_point_sample(points, num_centers) # B M
        cent_pts = gather_points(points.transpose(1,2).contiguous(), p_idx).transpose(1,2).contiguous() # B M 3
        cent_feats = gather_points(feats, p_idx) # B C M
    ...
    def get_ds_feats(feats, points, cent_pts, num_samples):
        ...
        cluster_feats = grouping_operation(feats, pn_idx) # B C M S
        cluster_pts = grouping_operation(points.transpose(1,2).contiguous(), pn_idx) # B 3 M S
    ...
    def forward(self, pts1, pts2, T_gt):
        ...
        if self.use_ppf:
            self.pts1 = pts1
            self.pts2 = pts2
            nm1 = self.emb_nm(pts1)
            nm2 = self.emb_nm(pts2)
            ...
            feats1 = self.emb_fpfh(pts1)
            feats2 = self.emb_fpfh(pts2)
